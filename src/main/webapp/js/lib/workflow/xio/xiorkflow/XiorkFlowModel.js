...
//
XiorkFlowModel.prototype.setAction = function (action) {
    this.action = name;
};
XiorkFlowModel.prototype.getAction = function () {
    return this.action;
};
...
// swimlane
XiorkFlowModel.prototype.addSwimlane = function (swimlane) {
	var index = this.indexOfSwimlane(swimlane);
	if(index>=0){
		this.swimlanes[index] = swimlane;
	}else{
		this.swimlanes[this.swimlanes.length] = swimlane;
	}
}

XiorkFlowModel.prototype.containsSwimlane = function(swimlane){
	for(i=0; i<this.swimlanes.length; i++){
		var swm = this.swimlanes[i];
		if(swm.name == swimlane.name){
			return true;
		}
	}
	return false;
...
XiorkFlowModel.prototype.indexOfSwimlane = function(swimlane){
	for(i=0; i<this.swimlanes.length; i++){
		var swm = this.swimlanes[i];
		if(swm.name == swimlane.name){
			return i;
...
function Swimlane(){
	this.name = "";
	// should be another object next step
	this.assignment = "";
}
